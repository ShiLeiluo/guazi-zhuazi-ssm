<%--
  Created by IntelliJ IDEA.
  User: SL
  Date: 2019/8/16
  Time: 16:28
  To change this template use File | Settings | File Templates.
--%>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<html>
<head>
    <meta charset="utf-8">
    <title>【北京二手车】北京二手车市场_个人二手车转让-瓜子二手车直卖网</title>
    <link rel="stylesheet" type="text/css" href="static/css/serve.css"/>
    <link rel="stylesheet" type="text/css" href="static/css/common.css"/>
    <link rel="shortcut icon" type="image/x-icon" href="static/img/favicon.ico" media="screen" />
</head>
<body>
<!-- 头部部分 -->
<header>
    <!-- 导航栏 -->
    <nav>
        <div class="nav-container">
            <div class="nav-left lf">
                <span class="lf"><a href="/"><img src="static/img/logo1_03.png" alt=""></a></span>
                <a href="" class="city lf">北京&nbsp;<img src="static/img/jiantou-down.png" alt=""></a>
            </div>
            <div class="nav-right rt">
                <ul>
                    <li><a href="/">首页</a></li>
                    <li><a href="/buy">我要买车</a></li>
                    <li><a href="/sell">我要卖车</a></li>
                    <li><a href="/serve">瓜子服务</a></li>
                    <li><a href="/financial">瓜子金融</a></li>
                    <li><a href="https://www.maodou.com/">毛豆新车</a></li>
                    <li><a href="">
                        <img src="static/img/login.png" alt="">
                        登录
                    </a></li>
                    <li>400-060-8029</li>
                </ul>
            </div>
        </div>
    </nav>
    <!--banner广告-->
    <div class="banner">
        <div class="banner-top">
            <img src="static/img/logoguazi.png" alt="" />
            <h1>平均每一分钟就成交一辆</h1>
        </div>
        <div class="banner-center">
            <h2>没有中间商赚差价&nbsp;<i>●</i>&nbsp;259项检测&nbsp;<i>●</i>&nbsp;30天可退&nbsp;<i>●</i>&nbsp;1年2万公里质保</h2>
        </div>
        <div class="banner-bottom">
            <a href="">售后咨询热线 400-066-8780</a>
        </div>
    </div>
</header>
<!--主体部分-->
<section>
    <!--交易流程-->
    <div class="dealFlow">
        <h1>直接和车主交易,没有中间商赚差价</h1>
        <img src="static/img/c2c-process.e1e9e70658e4097c7585d7988b995cb5.png"/>
    </div>
    <!--检测-->
    <div class="detection">
        <div class="detection-cont">
            <div class="detection-top">
                <h1>259项全车检测,安全</h1>
                <ul>
                    <li>
                        <h4>259项专业检测</h4>
                        <p>原奥迪中国二手车业务总监领衔制定</p>
                    </li>
                    <li>
                        <h4>完备的检测流程</h4>
                        <p>车辆上架前严格审核质量<br />成交过户前车况复检</p>
                    </li>
                    <li>
                        <h4>专业的评估师团队</h4>
                        <p>岗前培训<br />考核上岗 经验丰富</p>
                    </li>
                </ul>
                <button>排除重大事故</button>
            </div>
            <div class="detection-center">
                <ul>
                    <a href=""><li class="yuan"></li></a>
                    <a href=""><li></li></a>
                    <a href=""><li></li></a>
                </ul>
            </div>
            <div class="detection-bottom">
                <div class="exclude-left">
                    <ul>
                        <button>排除火烧车</button>
                        <p><b>&nbsp;&nbsp;√</b>&nbsp;&nbsp;&nbsp;&nbsp;发动机线束及橡胶制品无熏烟/火烧；</p>
                        <p><b>&nbsp;&nbsp;√</b>&nbsp;&nbsp;&nbsp;&nbsp;舱内保险丝盒熏烟/火烧；</p>
                        <p><b>&nbsp;&nbsp;√</b>&nbsp;&nbsp;&nbsp;&nbsp;车辆覆盖件及驾驶舱无熏烟/火烧；</p>
                        <p><b>&nbsp;&nbsp;√</b>&nbsp;&nbsp;&nbsp;&nbsp;防火墙无熏烟/火烧；</p>
                    </ul>
                    <ul style="margin-top:35px;margin-left:20px;">
                        <p><b>&nbsp;&nbsp;√</b>&nbsp;&nbsp;&nbsp;&nbsp;内饰无泥沙、水痕;</p>
                        <p><b>&nbsp;&nbsp;√</b>&nbsp;&nbsp;&nbsp;&nbsp;发动机舱无泥痕；</p>
                        <p><b>&nbsp;&nbsp;√</b>&nbsp;&nbsp;&nbsp;&nbsp;保险丝盒内无泥痕；</p>
                        <p><b>&nbsp;&nbsp;√</b>&nbsp;&nbsp;&nbsp;&nbsp;散热器无水痕；</p>
                    </ul>
                </div>
                <div class="exclude-right">
                    <ul>
                        <button>排除泡水车</button>
                        <p><b>&nbsp;&nbsp;√</b>&nbsp;&nbsp;&nbsp;&nbsp;音响喇叭底部无水痕；</p>
                        <p><b>&nbsp;&nbsp;√</b>&nbsp;&nbsp;&nbsp;&nbsp;驾驶舱地毯无水痕；</p>
                        <p><b>&nbsp;&nbsp;√</b>&nbsp;&nbsp;&nbsp;&nbsp;后备箱及备胎槽无水痕；</p>
                        <p><b>&nbsp;&nbsp;√</b>&nbsp;&nbsp;&nbsp;&nbsp;座椅滑轨内无水痕；</p>
                    </ul>
                    <ul style="margin-top:35px;margin-left:50px;">
                        <p><b>&nbsp;&nbsp;√</b>&nbsp;&nbsp;&nbsp;&nbsp;后排座椅无水痕；</p>
                        <p><b>&nbsp;&nbsp;√</b>&nbsp;&nbsp;&nbsp;&nbsp;安全带底部是否有水浸痕迹；</p>
                        <p><b>&nbsp;&nbsp;√</b>&nbsp;&nbsp;&nbsp;&nbsp;全车地胶地毯是否有水浸痕迹；</p>
                    </ul>
                </div>
            </div>
        </div>
    </div>
    <!--陪同交易-->
    <div class="accompany">
        <h1>陪同交易,省心</h1>
        <ul>
            <li>量身推荐车源</li>
            <li>销售陪同看车、协助议价</li>
            <li>提供过户服务</li>
            <li>瓜子担保支付</li>
        </ul>
        <img src="static/img/accompany.715e0f50f94320ddd1239270779e66c1.png"/>
    </div>
    <!--退换-->
    <div class="exchange">
        <div class="exchange-cont">
            <h1>30天可退1年质保,无忧</h1>
            <ul>
                <li>
                    <h4>重大事故30天可退</h4>
                    <img src="static/img/guarantee_02.png"/>
                    <p>担心买到问题车？<br />30天可退，瓜子二手车提供兜底保障</p>
                </li>
                <li>
                    <h4>1年2万公里质保</h4>
                    <img src="static/img/guarantee_05.png"/>
                    <p>二手车也有新车般保障<br />1年2万公里质保，买后无担忧</p>
                </li>
            </ul>
        </div>
    </div>
    <!--常见问题-->
    <div class="issue">
        <h1>常见问题</h1>
        <ul>
            <li>
                <!--问题logo-->
                <div class="issue-img">
                    <h3>Q1</h3>
                </div>
                <h4>瓜子二手车直卖网是谁?</h4>
                <p>
                    瓜子二手车直卖网是中国领先的二手车直卖平台，<br />平均每一分钟就成交一辆。<br />瓜子二手车撮合个人直接卖给个人，没有中间商赚差价。
                    <br />在瓜子平台上，车源均通过瓜子专业评估师的五维立体检测，<br />从源头保障车况。<br />用户在瓜子二手车购买车辆，
                    <br />均可享受重大事故30天可退，<br />1年2万公里质保服务，售后无忧。
                </p>
            </li>
            <li>
                <!--问题logo-->
                <div class="issue-img">
                    <h3>Q2</h3>
                </div>
                <h4>交易流程是怎么样的?</h4>
                <p>
                    买家攻略:<br />1、预约看车：拨打400-023-1529或在线预约；<br />2、专人带看：瓜子将安排售车顾问陪同您看车；<br />
                    3、签署合同：签署三方合同（买家、卖家、瓜子二手车），支付定金；
                    <br />4、交易过户：支付全款，好车开回家。<br />卖家攻略：<br />1、提交信息：填写表单或拨打400-023-1529预约
                    <br />2、上门服务：专业评估队伍免费上门登记验车<br />3、买家看车：交易顾问带领买家上门看车<br />4、交易过户：瓜子二手车专人帮忙办过户
                </p>
            </li>
            <li>
                <!--问题logo-->
                <div class="issue-img">
                    <h3>Q3</h3>
                </div>
                <h4>车辆过户怎么操作?</h4>
                <p>
                    我平台提供过户服务，
                    <br />
                    为您收集材料，指引协助过户。
                </p>
            </li>
            <li>
                <!--问题logo-->
                <div class="issue-img">
                    <h3>Q4</h3>
                </div>
                <h4>买车提供哪些保障?</h4>
                <p>
                    如发现车辆存在重大事故，承诺30天可退。
                    <br />
                    享受1年2万公里质保，养车无忧。
                </p>
            </li>
            <li>
                <!--问题logo-->
                <div class="issue-img">
                    <h3>Q5</h3>
                </div>
                <h4>是否收取费用?</h4>
                <p>
                    瓜子二手车仅收取4%的服务费，最低2500元。
                    <br />
                    （各个城市最低标准有差异，具体以车辆详情页为准）
                </p>
            </li>
            <li>
                <!--问题logo-->
                <div class="issue-img">
                    <h3>Q6</h3>
                </div>
                <h4>能提供什么样的金融服务?</h4>
                <p>
                    瓜子二手车有专属金融管家为您量身定制金融方案，
                    <br />
                    可拨打热线400-630-3009或在线快速申请。查看详情>
                </p>
            </li>
            <li>
                <!--问题logo-->
                <div class="issue-img">
                    <h3>Q7</h3>
                </div>
                <h4>页面上的价格还可以谈吗?</h4>
                <p>
                    页面上展示的价格是车主的报价。
                    <br />
                    瓜子二手车直卖网帮助买家直接和卖家面对面交易，
                    <br />
                    由买卖双方共同决定交易价格。
                </p>
            </li>
            <li>
                <!--问题logo-->
                <div class="issue-img">
                    <h3>Q8</h3>
                </div>
                <h4>提供车辆外迁服务吗?</h4>
                <p>
                    瓜子二手车暂未开通外迁服务业务，外迁可参照以下流程：
                    <br />
                    1.在车辆当前城市提档，与原车主交割车辆，办理临时牌；
                    <br />
                    2.了解迁入地准入标准，准备所需材料；
                    <br />
                    3.前往迁入地车管所，验车、登记并领取新牌。
                </p>
            </li>
        </ul>
    </div>
</section>
<!--底部部分-->
<footer	style="background-color:#2a2c37;">
    <div class="subject">
        <div class="top">
            <div class="left lf">
                <img src="static/img/global-3_03.png" class="logo lf" />
                <div class="phone lf">
                    <p>咨询电话：400-060-8029 &nbsp;&nbsp;（周一至周日 8:00-21:00）</p>
                    <p>服务监督热线： 400-066-8780  &nbsp;&nbsp;（周一至周日 9:00-21:00）&nbsp;&nbsp;</p>
                    <br>
                    <p>互联网违法或不良信息举报联系方式： </p>
                    <p>邮箱： jubao@guazi.com  电话：010-89191670</p>
                </div>
                <div class="lf">
                    <img src="static/img/ewm-wx-2.png" alt="">
                    <p style="text-align:center;">关注微信</p>
                </div>
                <div class="lf" style="margin-left:30px;">
                    <img src="static/img/ewm-app-2.png" alt="">
                    <p style="text-align:center;">下载APP</p>
                </div>
            </div>
            <div class="right rt">
                <ul>
                    <li><a href="">关于瓜子</a></li>
                    <li><a href="">加入我们</a></li>
                    <li><a href="">联系我们</a></li>
                    <li><a href="">服务保障</a></li>
                    <li><a href="">隐私声明</a></li>
                    <li><a href="">服务条款</a></li>
                </ul>
                <p>Copyright 2018 www.guazi.com All Rights Reserved</p>
                <p>京ICP备15053955号 ICP证151071号</p>
                <p style="	margin-top:5px;">
                    <a href="">
                        <img src="static/img/icon-foot_03_02.png" alt="">
                        京公网安备11010802020161号
                    </a>
                </p>
                <p style="margin-top:5px;">
                    <img src="static/img/icon-foot_01.png" alt="">
                    <img src="static/img/icon-foot_02.png" alt="">
                </p>
            </div>
        </div>
        <div class="bottom">
            <ul>
                <h4>热门品牌：</h4>
                <li style="	margin-left:10px;"><a href="">北京二手大众</a></li>
                <li><a href="">北京二手奔驰</a></li>
                <li><a href="">北京二手宝马</a></li>
                <li><a href="">北京二手奥迪</a></li>
                <li><a href="">北京二手现代</a></li>
                <li><a href="">北京二手福特</a></li>
                <li><a href="">北京二手别克</a></li>
                <li><a href="">北京二手丰田</a></li>
                <li><a href="">北京二手日产</a></li>
                <li><a href="">北京二手本田</a></li>
                <li><a href="">北京二手路虎</a></li>
                <a href="" class="rt"><img src="static/img/global-3_04.png" alt=""></a>
            </ul>
            <br>
            <ul>
                <h4>热门车系：</h4>
                <li style="	margin-left:10px;"><a href="">北京二手福特福克斯</a></li>
                <li><a href=""> 北京二手奥迪奥迪A6...</a></li>
                <li><a href=""> 北京二手奥迪奥迪A4...</a></li>
                <li><a href=""> 北京二手奥迪奥迪Q5</a></li>
                <li><a href=""> 北京二手大众迈腾</a></li>
                <li><a href=""> 北京二手大众速腾</a></li>
                <li><a href=""> 北京二手大众途观</a></li>
                <li><a href="">北京二手大众高尔夫</a></li>
                <a href="" class="rt"><img src="static/img/global-3_04.png" alt=""></a>
            </ul>
            <br>
            <ul>
                <h4>友情链接：</h4>
                <li style="	margin-left:10px;"><a href="">物流百科</a></li>
                <li><a href="">  赶集二手车</a></li>
                <li><a href="">  北京二手车</a></li>
                <li><a href="">  北京天气预报15天</a></li>
                <li><a href="">  北京装修网</a></li>
                <li><a href="">  北京个人二手车</a></li>
                <li><a href=""> 违章查询</a></li>
                <li><a href=""> 北京车市</a></li>
                <li><a href=""> 北京二手车</a></li>
                <li><a href=""> 北京信息港</a></li>
                <li><a href=""> 北京百姓网</a></li>
                <li><a href="">  车讯网</a></li>
                <a href="" class="rt"><img src="static/img/global-3_04.png" alt=""></a>
            </ul>
            <br>
            <ul>
                <h4>热门城市：</h4>
                <li style="	margin-left:10px;"><a href="">全国二手车</a></li>
                <li><a href="">  北京二手车</a></li>
                <li><a href="">  郑州二手车</a></li>
                <li><a href="">  昆明二手车</a></li>
                <li><a href="">  沈阳二手车</a></li>
                <li><a href="">  成都二手车</a></li>
                <li><a href=""> 上海二手车</a></li>
                <li><a href=""> 南宁二手车</a></li>
                <li><a href=""> 广州二手车</a></li>
                <li><a href=""> 重庆二手车</a></li>
                <li><a href=""> 天津二手车</a></li>
                <li><a href="">深圳二手车</a></li>
                <a href="" class="rt"><img src="static/img/global-3_04.png" alt=""></a>
            </ul>
            <br>
            <ul>
                <h4>移动版：</h4>
                <li style="	margin-left:10px;"><a href="">瓜子二手车北京移动页</a></li>
            </ul>
            <br>
            <ul>
                <h4>热门页面：</h4>
                <li style="	margin-left:10px;"><a href="">1万元二手车</a></li>
                <li><a href="">  2万元二手车</a></li>
                <li><a href="">  3万元二手车</a></li>
                <li><a href="">  4万元二手车</a></li>
                <li><a href="">  5万一下二手车</a></li>
                <li><a href="">   二手越野车</a></li>
                <li><a href="">二手商务车</a></li>
                <li><a href="">  二手跑车</a></li>
                <li><a href="">  自动挡二手车</a></li>
                <li><a href=""> 急卖二手车</a></li>
                <li><a href=""> 低价出售二手车</a></li>
                <a href="" class="rt"><img src="static/img/global-3_04.png" alt=""></a>
            </ul>
            <br>
            <p>车好多旧机动车经纪（北京）有限公司    北京市丰台区南四环中路260号北京新发地汽车交易市场旧车经营区B区178号    010-57317000</p>
        </div>
    </div>
</footer>
</body>
</html>
